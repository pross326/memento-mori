    ...
    <v-row class="ma-8">
      ...
      <v-col cols="3">
        ...
        <v-card class="rounded-lg mt-5 mb-5">
          <v-card-title class="text-h5">Age: {{ weekCalculator}}</v-card-title>
        </v-card>
        <v-card class="rounded-lg mt-5 mb-5">
          <v-card-title class="text-h5">Predicted Lifespan: {{ value }} Years</v-card-title>
        </v-card>
        <v-card class="rounded-lg">
          <v-card-title class="text-h5">Weeks Remaining: {{ weeksRemaining }} weeks</v-card-title>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
...
export default {
  ...
  computed: {
    ...
    weekCalculator(dt1, dt2){
      ...
      let birthYear = Number(this.picker.substr(0,4));
      if (this.picker == today){
        return "Enter your birthday to calculate your age in weeks"
      }
      else{
        ...
        return `You have lived ${weeksOld} weeks.`;
    }},
    weeksRemaining(){
